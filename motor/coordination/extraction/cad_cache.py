"""Content-addressed JSON cache for local CAD coordination extractors."""
...
def file_cache_key(path: Path) -> str:
    """Content-based cache key so staged copies reuse extractions."""
    stat = path.stat()
...
def cache_json_path(cache_root: Path, *, key: str, suffix: str) -> Path:
    ...
    return cache_root / f"{key}.{suffix}.json"
