    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      {!isHub && active ? (
        <div
          ...
          className="flex shrink-0 flex-wrap items-center gap-3 rounded-2xl border border-black/10 bg-white px-4 py-3 shadow-(--shadow-card)"
          role="region"
          aria-label="Volver al inicio y título de sección"
        >
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-xl border border-black/12 bg-white px-3 py-2 text-sm font-semibold text-primary shadow-sm outline-none transition hover:bg-primary/6 focus-visible:ring-2 focus-visible:ring-primary/35 focus-visible:ring-offset-2"
      ...
      >
        {!isHub ? (
          <div className="rounded-2xl border border-black/10 bg-white p-4 shadow-(--shadow-card) sm:p-5">
            {children}
          </div>
        ) : (
          children
