  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 z-0" aria-hidden>
        <div className={`absolute inset-0 bg-linear-to-br ${gradientForKind(project.project_kind)}`} />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_22%_18%,rgba(255,255,255,0.35),transparent_42%),radial-gradient(circle_at_82%_28%,rgba(255,200,100,0.28),transparent_38%)]" />
      </div>

      <div className="du-folder-card-heading pointer-events-none">
        <h3 className="du-folder-card-title">{project.name}</h3>
        <p className="du-folder-card-subtitle">{subtitle}</p>
      </div>

      <div className="pointer-events-none absolute right-5 top-4 z-20 text-right text-white drop-shadow-md">
      ...
      <div className="du-folder-tab-clip pointer-events-none absolute bottom-0 left-0 right-0 z-10 h-[56%]">
        <div className="absolute inset-0 bg-black/40" aria-hidden />
      </div>

      <div className="absolute bottom-0 left-0 right-0 z-20 h-[56%] pb-4 pr-4 pl-4">
        <button
          type="button"
          className="pointer-events-auto absolute right-3 top-3 rounded-lg p-1.5 text-white/45 outline-none transition-colors duration-200 hover:bg-white/10 hover:text-white"
        ...
        {menuOpen ? (
          <div
            className="pointer-events-auto absolute right-3 top-10 z-30 w-40 rounded-xl border border-black/10 bg-white py-1 shadow-lg"
        ...
        <span className="pointer-events-none absolute left-4 top-3 inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white/85 backdrop-blur-sm">
          <ProjectKindIcon kind={project.project_kind} />
          {dashboardBucketLabel(bucket)}
        </span>

        <div className="pointer-events-none absolute right-4 bottom-3.5 left-4 space-y-1.5">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-baseline gap-1.5 text-[10px]">
              <span className="font-semibold uppercase tracking-wider text-white/35">Progreso</span>
